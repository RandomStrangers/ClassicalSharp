...
namespace Launcher {
	...
	public static class Client {
		...
		public static bool Start(ClientStartData data, ref bool shouldExit) {
			if ((DateTime.UtcNow - lastJoin).TotalSeconds < 1)
				return false;
            lastJoin = DateTime.UtcNow;
			
			string skinServer = "http://cdn.classicube.net/skin/";
			string args = data.Username + " " + data.Mppass + " " +
				data.Ip + " " + data.Port + " " + skinServer;
			return StartImpl(data, args, ref shouldExit);
		}
		
		public static bool Start(string args, ref bool shouldExit) {
			return StartImpl(null, args, ref shouldExit);
		}

        public static bool StartImpl(ClientStartData data, string args, ref bool shouldExit) {
			if (!Platform.FileExists(GetExeName())) return false;
			
			UpdateSettings(data, out shouldExit);
			try {
				StartProcess(args);
        ...
        public static bool NeedOSXHack() {
        ...
        public static void UpdateSettings(ClientStartData data, out bool shouldExit) {
			shouldExit = false;
